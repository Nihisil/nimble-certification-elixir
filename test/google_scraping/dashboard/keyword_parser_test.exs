  ...
  describe "parse/1" do
    test "given HTML, returns parsed results" do
      use_cassette "google/crawl_success" do
        ...
        assert parsed_results.ad_top_urls_count == 1
        assert parsed_results.ad_total_count == 2
        assert parsed_results.non_ad_results_count == 9
        assert parsed_results.non_ad_results_urls_count == 9
        assert parsed_results.total_urls_count == 76
        assert length(parsed_results.urls) == 76
      end
    end
  end
end
